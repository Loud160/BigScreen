...
Network access is limited to the following user-facing video and update
features:
...
- **Check Big Screen** contacts `api.github.com/repos/Loud160/BigScreen/releases/latest` once in the background when Big Screen first opens during a game session, or when the player presses the manual update button. This check reads release metadata only; Big Screen does not download or install its own updates.
