...
| 6 | Packaging, CI, and dependency reproducibility | Validated for alpha release | `b848661` on `codex/fable-review-stage-6` | Canonical QMOD validation passed; MBF package/install behavior was exercised during subsequent alpha testing, while the final tagged artifact remains reproducibly generated by CI |
...
| 8 | Catalog lifetime and transport-state consolidation | Quest validated | `5f8c802` on `codex/fable-review-stage-8` | Combined Stage 8/9 testing completed without a reported regression, followed by several days of use alongside Saber Stage and Qavatars |
| 9 | Documentation and contained low-risk cleanup | Quest validated | `0ade4a0` on `codex/fable-review-stage-9` | Normal and diagnostic ARM64 builds passed; the ordinary QMOD excludes the deliberate-crash control, and the combined headset pass found no regression |
...
The combined end-to-end Stage 8/9 pass was subsequently completed by the user.
On September 6, 2026, the user also reported three to four days of testing the
current main-branch build alongside Saber Stage and Qavatars without finding an
issue. This closes the review's staged Quest-validation requirement for the
alpha. It does not convert the alpha test record into a claim of exhaustive map,
mod, or headset coverage.
...
That completion rule was satisfied for the 0.7.0-alpha.13 release candidate on
September 6, 2026. Wider community testing continues under the project's alpha
status and the boundaries documented in `docs/KNOWN_ISSUES.md`.
