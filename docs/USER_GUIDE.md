...
**Misc > Performance > GPU Video Conversion** is a default-off experiment. It keeps the same decoder, synchronization, layouts, mapper effects, and Showcase choreography, but uploads reusable Y/U/V planes and performs conversion, container rotation, color correction, and vignette in one GPU pass. It remains a CPU-readable MediaCodec path rather than zero-copy. Unsupported frame layouts or unavailable Unity resources automatically return that playback session to CPU RGBA and record the reason. Thumbnails are unchanged. Compare it on Quest 2 with the same map, file, FPS limit, headset temperature, and graphics settings before deciding whether it helps a workload.
