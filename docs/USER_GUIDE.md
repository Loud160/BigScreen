...
The playback group starts/stops both the map audio and video. Drag the scrubber to seek; the centered time shows current position and duration. Play holds the song clock for 250 ms while the decoder works ahead; this is a non-blocking menu-preview pre-roll, not an added video offset. At the natural end, the scrubber returns to zero and the synchronized song/video preview loops through the same 250 ms pre-roll. Big Screen clears the completed decoder state and waits for a newly decoded opening picture before restarting the song; Pause/Play after completion uses the same restart path. Long-running previews retain compressed packets while MediaCodec is busy and fully drain asynchronous final frames before looping, preventing intermittent freezes near the end.
...
**Misc > Performance > GPU Video Conversion** defaults on. It keeps the same decoder, synchronization, layouts, mapper effects, and Showcase choreography, but uploads reusable Y/U/V planes and performs conversion, container rotation, color correction, and vignette in one GPU pass. It remains a CPU-readable MediaCodec path rather than zero-copy. Unsupported frame layouts or unavailable Unity resources automatically return that playback session to CPU RGBA and record the reason. Thumbnails are unchanged. Existing installations are switched on once when they first load the new settings format; after that migration, the user's choice is preserved normally.

**Consolidated YUV Upload** is a second default-off A/B switch below GPU Video Conversion. It writes the same 8-bit 4:2:0 picture into one packed atlas so Unity performs one texture upload instead of three. If packed setup fails, playback remains on GPU through the established 3-plane method. The performance panel shows the active path as `GPU YUV Packed` or `GPU YUV 3-Plane`, and the log records any fallback reason. Only a later failure of GPU conversion itself selects CPU RGBA.

**GPU Read-Ahead Memory** appears directly below that switch. Its 32–256 MiB budget controls how many future YUV pictures the worker may prepare, with 64 MiB as the default. More memory gives short 1440p/60 decoder stalls a larger reserve but does not correct sustained overload. It is allocated on demand and applies when the next video opens. Play/Practice transitions opportunistically fill the reserve before the gameplay clock begins; Big Screen never waits for a full queue before starting. Leaving a selected map, completing/exiting/restarting gameplay, looping, or moving the Video Library scrubber invalidates the old reserve before another picture can be presented.

Show Performance Information displays the actual presentation-oriented video resolution, source FPS, current FPS limit, frames skipped, true session-average/peak frame-preparation CPU time, and automatic reductions during gameplay and on the results/failure screen. Frames Skipped is a cumulative count of presentation deadlines missed during the current playback session, so a later decoder catch-up cannot make the value decrease. Automatic Performance separately evaluates a bounded recent window, allowing it to recognize recovery without rewriting that session total. The append-only log records asynchronous decoder-worker wait time separately rather than presenting it as processing cost. The movable panel shares one saved position and angle between the Video Library and gameplay; placement is committed when the panel is disabled or its current menu/gameplay context closes. Use the adjacent circular reset button if the panel is moved out of reach.

When performance logging is enabled, the completed-gameplay log also includes decoder startup time. GPU YUV sessions additionally record the read-ahead memory budget, frame capacity, peak/final reserve, low/empty reserve transitions, bounded catch-up presentations, forced late drops, and the peak number of pictures simultaneously due. Ending a GPU menu preview writes the same queue summary to Big Screen's normal log so preview tests remain diagnosable. A catch-up presentation means the 90 Hz menu or gameplay update cadence recovered one slightly late picture without allowing the video clock to drift; a forced late drop means the backlog was already too old or too large to recover safely. These measurements stay in logs rather than the live panel because they are evidence for comparing timing behavior, not presentation statistics a player needs to watch during a map.

Record Power Benchmark creates two analysis files under `/sdcard/ModData/com.beatgames.beatsaber/BigScreen/Logs`: `power-benchmark-summary.csv` has one row per map, while `power-benchmark-samples.csv` has the one-second observations. It records baseline maps even when Video In Map is off, so compare otherwise identical runs by the `video_active` column; `decode_method` identifies hardware, software, or no active video decoder. Whole-process CPU includes Beat Saber and every loaded mod; `decoder_cpu_ms` isolates Big Screen's owned decoder worker, including CPU YUV-to-RGBA conversion when that path is active. Main-thread plane/RGBA upload, optional GPU conversion, and screen rendering remain part of the whole-process difference rather than being falsely attributed to H.264 decoding. If the CSV schema changes, Big Screen preserves the older file with a timestamped `-legacy-` name before starting the new schema.
...
The Update tab shows the installed Big Screen version and the yt-dlp package currently active in the embedded downloader. Its nightly switch reflects the channel actually loaded, rather than a pending preference; a staged channel change appears only after the replacement activates and passes its startup test on the next Beat Saber launch. Big Screen checks the latest public stable mod release once when its menu first opens during each Beat Saber session. A newer version produces one popup showing the installed and available versions; reopening the menu does not repeat that automatic check. **Check Big Screen Update** always performs a fresh manual check and reports whether an update exists, the installed version is current, or the check could not be completed. Mod updates remain user-installed through ModsBeforeFriday or GitHub rather than being silently installed in-game.
