...
Show Performance Information displays the actual presentation-oriented video resolution, source FPS, current FPS limit, missed frames, decode delay, and automatic reductions during gameplay and on the results/failure screen. Its movable panel shares one saved position and angle between the Video Library and gameplay; placement is committed when the panel is disabled or its current menu/gameplay context closes. Use the adjacent circular reset button if the panel is moved out of reach.
