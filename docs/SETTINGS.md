...
The append-only performance log also records how long the decoder took to open. That startup measurement is intentionally not displayed on the live panel because it is a one-time session value rather than a changing playback statistic.
