...
### Use Nightly yt-dlp — default: Off

Shows the yt-dlp channel that the embedded Python runtime actually loaded. Enabling requires confirmation because nightly builds receive fixes sooner but are more likely to contain regressions. Turning the switch on or off starts a release check for the requested channel; it does not claim that the channel changed immediately. A downloaded replacement remains staged until Beat Saber restarts, and the switch changes only after Big Screen activates, imports, and smoke-tests the package successfully. This option affects downloader updates only, not Big Screen updates.
...
The main check button follows the currently loaded stable/nightly channel. While a nightly package is active, **Check Stable Release** provides an explicit path back to the recommended stable channel. Big Screen compares the stable release date with the installed nightly: it says when stable has caught up, or warns that switching would install an older release when it has not. That warning is informational and never blocks a deliberate return to stable. The stable and nightly check buttons use the same size and visual weight. Manual results open over the Update tab that requested them and are explicitly ordered in front of that panel. Installation downloads the official asset, verifies it against the official SHA-256 list, validates the archive, and stages it for the next Beat Saber restart. It never replaces a running Python module or changes the displayed channel before activation succeeds.
...
Available only when GPU Video Conversion is on. This A/B experiment packs luma above side-by-side U/V regions in one reusable R8 atlas. The decoder worker writes the atlas directly, so Unity presents each picture with one `LoadRawTextureData()` and one `Texture2D.Apply()` instead of three without adding a main-thread repack. The same conversion shader operations and stable shared RGB RenderTexture preserve ordinary, curved, mapper-authored, and Showcase screens.

If the packed shader, atlas allocation, or atlas layout cannot be used, Big Screen first continues through the proven `GPU YUV 3-Plane` method; only a separate failure of that method falls back to `CPU RGBA`. The performance panel's Decoder row and the performance log report `GPU YUV Packed`, `GPU YUV 3-Plane`, or the applicable CPU path. The log also records why a requested packed session fell back. Changing this switch recreates an active Video Library preview and applies to gameplay when the next video opens.

#### GPU Read-Ahead Memory — default: 64 MiB, range: 32–256 MiB

Available when GPU Video Conversion is on. This 16 MiB-step slider limits the memory that may be occupied by decoded future YUV pictures. Higher values can absorb longer temporary decoder/scheduling stalls, especially for 1440p/60 FPS, but increase Beat Saber's memory use while a video is open. Big Screen allocates frames only as needed; it does not reserve the complete amount in advance. A separate 120-frame safety ceiling prevents low-resolution media from running unreasonably far ahead. The new budget applies to the next opened preview or map and never restarts a video while the slider is being adjusted.

Gameplay prewarming begins filling this reserve during the normal transition after Play or Practice is selected, without blocking the song or Unity thread. The complete queue is invalidated on a timeline scrub, loop/restart, playback-cadence change, selected-map change, decoder fallback, normal completion, and early exit. Completed gameplay writes the selected budget, calculated frame capacity, peak/final depth, and low/empty reserve-transition counts to the append-only performance log when performance logging is enabled. Menu-preview teardown writes the same one-line queue summary to the normal mod log so preview comparisons can also be diagnosed. These queue counters are intentionally not added to the live panel.
...
Displays the active hardware/software backend, presentation-oriented native video resolution, source FPS, current FPS limit, expected versus presented source frames, missed percentage, true session-average/peak frame-preparation CPU time, and automatic reductions during gameplay. Hold the trigger anywhere on the panel to move and angle it. Big Screen saves that transform when the switch is turned off or the menu/gameplay context closes, and the same placement is reused in the Video Library and during video maps. The circular reset button immediately to the left of this switch returns the panel to its safe default position and angle. The results/failure summary also identifies the loaded FFmpeg runtime, CPU RGBA/GPU YUV presentation method, main-thread presentation time, and reusable frame-buffer allocation count. Enable this when comparing decoder builds, tuning frame rate, or reporting performance problems.

The append-only performance log also records how long the decoder took to open and separately reports worker wall-clock wait time. Wait time includes asynchronous MediaCodec output waits and decoder-thread descheduling; it is not labeled as decoder processing cost. These values are intentionally omitted from the live panel because preparation CPU is the useful changing workload statistic and startup is a one-time session value.

#### Record Power Benchmark — default: Off

Records one sample per second for every played map, including maps played with **Video In Map** off. Each sample contains the Quest battery charge counter, instantaneous and Android-averaged battery current when the headset exposes them, battery percentage/charging state, whole Beat Saber process CPU time, Big Screen decoder-worker CPU time, actual video resolution, active FPS limit, and playback statistics. Preparation CPU and worker wait use separate CSV columns. Data stays in memory during gameplay and is appended to CSV files only after the map ends or is exited. When a release changes the CSV schema, an older CSV is preserved with a timestamped `-legacy-` filename.
