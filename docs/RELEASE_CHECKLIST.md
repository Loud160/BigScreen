...
- [ ] Menu Background Opacity defaults to 0% and changes live at 0%, an intermediate value, and 100%. Verify the Settings, song browser, video editor, storage/showcase, local-video browser, thumbnail picker, and Advanced Sync pages all use the same black backing level without intercepting controls or restarting preview playback. Reset to Defaults must return to 0%; closing and reopening the menu must preserve any saved nonzero value. Modal cards keep their own backgrounds and the empty center preview controller remains clear.
