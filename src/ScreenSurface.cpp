    ...
    namespace {
        ...
        UnityEngine::Shader* FindVideoShader()
        {
            ...
            try
            {
                ...
                if(!loaded)
                {
                    // The asset path is retained as a compatibility fallback
                    // for bundles regenerated by older Unity editor tooling.
        ...
        VideoShaderChoice ResolveVideoShader(bool pictureTransparent)
        {
            ...
            if(preferEmbedded)
            {
                if(auto* bloomSafe = FindVideoShader())
                {
                    if(announcedFamily !=
            ...
            if(auto uiShader = UnityEngine::Shader::Find("UI/Default"))
            {
                   ...
                   static_cast<int>(VideoShaderFamily::UiMasked))
                {
                    ...
                    {
                        PaperLogger.error(
                            "The embedded video shader did not load; using "
                            "the UI/Default RGB-only method instead.");
                        // error-history.log is the only log the support
                        // collector reliably captures fresh, so a shader
                        // method that differs from the player's selection
                        // must be visible there or a field report cannot
                        // tell which method produced the reported picture.
                        ErrorManager::Instance().RecordError(
                            "Selecting the video screen shader",
                            "The embedded video shader did not load; using "
                            "the UI/Default RGB-only method");
                    }
                    else
                    {
                        PaperLogger.info(
        ...
        bool prepareDeformation)
    {
        ...
        if(!CreateMesh(config, aspectRatio) ||
           ...
           !CreateMaterialAndTexture(
               videoWidth, videoHeight, config, sharedTexture) ||
